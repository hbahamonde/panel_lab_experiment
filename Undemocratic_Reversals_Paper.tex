...
\date{\textbf{Principal Investigator and project contact:} Janne Tukiainen, Professor, Turku School of Economics, University of Turku; \href{mailto:janne.tukiainen@utu.fi}{janne.tukiainen@utu.fi}\\[0.4em]Working proposal --- \today}
...
Participants receive a EUR~10 show-up payment plus a points-based performance bonus capped at EUR~5. For 300 completed participants, total compensation cannot exceed EUR~4,500. At completion, the interface displays the selected Block-1 round, selected Block-2 round, point total, performance payment, participation payment, and total payment separately.
...
The operational protocol fixes the completed roster before Block 1 and forms the pool configuration in \autoref{tab:attendance-pools}. No participant enters or changes pools after paid play begins. Strategic decisions do not time out automatically: the software records response time and flags delays beyond 90 seconds without generating an answer. The laboratory manager monitors these flags and prompts delayed participants to complete the current decision. The session log must document late arrival, browser failure, early departure, manual intervention, and payment exceptions. Exclusion rules should be preregistered and should never depend on treatment-consistent behavior.

Before launch, two pilots are required. A technical pilot should test 10- and 15-person pools, mixed-pool sessions, both treatment branches, wait pages, rematching, delayed-response monitoring, refresh persistence, and payment calculations. A behavioral pilot should estimate duration, comprehension, Block-1 \Aproc{} support, approval, rejection, transfers, fatigue, and payoff distributions. Parameters should be frozen after that pilot and before confirmatory registration.
...
\paragraph{Procedural familiarity.} Participants may initially misunderstand payoff rules. Guided unpaid practice and mandatory comprehension checks establish familiarity before paid decisions. Block-2 Round 1 records the immediate response, while Round 10 records sustained reversal and Rounds 8--10 summarize late behavior.

\paragraph{Reputation and reciprocity.} Fixed five-person groups would allow personal histories to drive votes and contributions. Anonymous rematching within 10- or 15-person pools breaks stable bilateral relationships while keeping treatment exposure and synchronized interaction feasible. Participants never receive persistent identities.

\paragraph{Leader-role expectations.} Leader selection occurs after the institutional ballot and uses an independent uniform draw among the five citizens under both procedures. Each citizen therefore has a one-in-five chance in every round, cannot vote knowing whether they will make the proposal, and cannot use prior service to predict the next leader.
...
\paragraph{Cross-block comparability.} Labels, leader selection, proposal choices, approval threshold, fallback, endowments, and sequence are fixed. Recovery jointly improves public-service conditions and raises the approval-constrained multiplier; persistence identifies change from repeated play or approaching the session's end.
...
The experiment makes action without approval materially effective but avoids moral labels. Instructions use ``Group approval required'' and ``Decision takes effect directly,'' disclose every rule, and reserve explicit questions about democratic constraints until paid choices are complete.

Registration through the host laboratory's participant system covers that laboratory's general rules and privacy notice. Before practice, the interface presents study-specific information on the session, interaction, anonymity, payment, data linkage, and the importance of remaining for the full task, then records affirmative consent to this study. Participants who do not consent are handled by the laboratory manager before the group task begins. The debrief explains the democratic interpretation only after all incentivized decisions and clarifies that no single vote or transfer establishes that a participant is corrupt, authoritarian, or opposed to democracy. Data exports use coded study IDs; the host laboratory keeps any temporary payment linkage separate and does not give it to the research team.

The Finnish setting strengthens the case for a hypothetical, incentive-based task. Participants do not judge Finnish officials; they face a fictional crisis and transparent, consequential rules. Recovery is the joint improvement in that environment and the approval-constrained production rate.

The interpretation centers on revealed choice over approval. Comparing recovery with persistence identifies whether the structural package causes previous \Aproc{} supporters to restore that constraint; it does not separate the package's narrative and payoff components.

\section{Planned Deliverables and Decision Rule}

The principal conclusion follows a simple rule: evidence for structural reversal requires a positive, substantively meaningful, and statistically distinguishable recovery effect on $R_i$ among Block-1 \Aproc{} supporters. Approval, proposals, transfers, fallback contributions, and expectations describe behavior around that effect.

The design isolates a microfoundation of executive aggrandizement: citizens use a democratic ballot to remove approval from a later decision and may restore it after structural recovery. Democratic reversal is the sustained vote to reinstate that constraint.

...
Second, it identifies a structural cause of reversal. All participants encounter a public-service crisis in Block 1: ``{\bf Imagine that you are a citizen in a society where public services have been under serious strain for a long time. Parliament has failed to pass a budget for eight months, healthcare waiting times have doubled, and planning for public services has stalled}.'' For Block 2, matched pools are randomly assigned either to recovery, in which the crisis recedes, or to persistence, in which it continues. The leader-led institution itself does not improve or deteriorate. Consequently, {\bf the treatment changes the instrumental case for concentrating power rather than supplying a moral appeal to democracy}.

\begin{plainlanguagebox}
This proposal asks whether citizens remain committed to democratic control when a crisis makes one-person rule appear faster and more effective. The undemocratic act is not simply selecting an undemocratic leader. It is knowingly accepting a rule under which one person replaces collective choice and can impose a decision on everyone without prior group approval. The option to move points into a personal account is an incentive device: {\bf it gives concentrated authority a real, payoff-relevant temptation and makes its consequences material}. This reveals how citizens and selected leaders behave when collective control competes with effectiveness and individual reward.

The experiment then asks whether support for one-person rule survives after the crisis recedes. Unlike other designs, recovery is not merely described in a vignette: it changes the payoff structure, raising the return from citizen-controlled provision from 1.50 to 2.50 so that it matches the leader-led method, while the leader's unilateral authority and personal-transfer option remain unchanged. If the same citizens return to collective decision making once concentrating power no longer offers a practical advantage, they reverse their earlier support for a backsliding arrangement. In short: \emph{when the crisis no longer rewards giving up collective control, do citizens take that control back?}

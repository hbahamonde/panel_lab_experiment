...
\begin{abstract}
PENDING \\ {\vspace{5cm}\color{red}See prototype here}: \url{https://panel-lab-experiment.onrender.com}
\end{abstract}
...
Second, it identifies a structural cause of reversal. All participants encounter a public-service crisis in Block 1: ``{\bf Imagine that you are a citizen in a society where public services have been under serious strain for a long time. Parliament has failed to pass a budget for eight months, healthcare waiting times have doubled, and planning for public services has stalled}.'' For Block 2, matched pools are randomly assigned either to recovery, in which the crisis recedes, or to persistence, in which it continues. The leader-led institution carries the same multiplier, authority, and personal-transfer rule across both blocks. The treatment therefore isolates the crisis-created productivity premium attached to concentrating power.

Third, it identifies changes in \emph{revealed institutional preference}. Democratic commitments acquire their meaning through consequential choices. During the crisis, a vote for one-person rule can express a willingness to exchange collective control for productive performance. Recovery changes that bargain by raising citizen-controlled productivity to the leader-led rate. Leader control still offers compulsory coordination: one person can require equal allocations and thereby overcome voluntary free-riding. A citizen who returns to collective choice therefore reveals a preference for democratic control even when one-person rule may coordinate provision more successfully. This is a demanding form of democratic reversal: the same citizen accepts the strategic risks of collective choice once the crisis-created productivity premium for concentrating power has expired.

\newpage
\begin{plainlanguagebox}
This proposal asks whether citizens remain committed to democratic control when a crisis makes one-person rule appear faster and more effective. The undemocratic choice occurs when citizens knowingly accept a rule under which one person replaces collective choice and can impose a decision on everyone without prior group approval. The option to move points into a personal account gives concentrated authority a payoff-relevant temptation and makes its consequences material. The game thereby reveals how citizens and selected leaders behave when collective control competes with effectiveness and individual reward.

The experiment then asks whether support for one-person rule survives after the crisis recedes. Recovery directly changes the payoff structure: the return from citizen-controlled provision rises from 1.50 to 2.50 and matches the leader-led rate. The leader retains the ability to require equal allocations and coordinate provision, together with the option to move fund points into a personal account. Citizens who return to collective decision making therefore reclaim democratic control while accepting the continuing risk of voluntary free-riding. In short: \emph{when one-person rule loses its crisis-created productivity advantage, do citizens value collective control enough to take it back?}
...
In the experiment, the citizen-led procedure (\Dproc) preserves individual control: each person decides how many points to place in a public-services fund, and no one can remove points from that fund for personal use. Its low Block-1 productivity, however, makes public-service provision difficult. The leader-led procedure (\Aproc) represents a faster coordinated response: one person makes the decision for all five citizens, and the remaining fund is converted into public services more effectively. The same leader may also move part of the fund to a personal account. The productive multiplier gives this faster-response advantage a consequential payoff counterpart. Support for \Aproc{} can therefore express an instrumental response to poor performance during a prolonged public-service crisis.

\paragraph{H1: Crisis endorsement.} In Block 1, support for the leader-led method will increase when participants' experience of its productivity premium and compulsory coordination outweighs the risk of a personal transfer.
...
The central theoretical claim is conditional. If support for leader discretion was learned from weak structural performance, then credible improvement should change its relative appeal. Before Block 2, recovery raises the citizen-led method's productivity to the leader-led method's level. This removes the exogenously imposed productivity premium for one-person rule while preserving its compulsory coordination and the leader's discretion to move fund points into a personal account. Citizens can then transform every contributed point into public services at the same rate under either method, while only the leader can require everyone to contribute. The resulting choice retains real economic stakes: collective control carries the risk of free-riding, whereas leader control carries the loss of individual choice and the possibility of a personal transfer. Returning to \Dproc{} under these conditions reveals a willingness to restore democratic control despite the leader-led method's remaining coordination advantage.
...
\paragraph{H3: Structural updating.} Participants assigned to recovery will report a larger increase in perceived citizen-led effectiveness and a larger reduction in the perceived need for leader control than participants assigned to persistence.
...
\label{eq:democratic}
\end{equation}
The marginal private return is below 1 in both public-service environments, preserving the social dilemma, while the group return exceeds 1. In Block 1, $m_{g1}=1.50$ for everyone. In Block 2, $m_{g2}=2.50$ under recovery and $m_{g2}=1.50$ under persistence.
...
\paragraph{How incentives map onto reversal.} The treatment implements the changing social context through the relative productivity of the two decision methods. During the crisis, the citizen-led fund uses \(m=1.50\), whereas the leader-led fund uses \(m=2.50\). Under recovery, the citizen-led multiplier rises to \(2.50\) and the leader-led rule remains unchanged. The persistence condition retains the crisis incentive structure in Block 2, providing the counterfactual for changes caused by repeated play, fatigue, or end-game behavior.

\Cref{tab:payoff-matrix} illustrates the comparison by supposing that ten of each citizen's twenty points enter the fund and that the leader makes no personal transfer. Holding the fund allocation fixed isolates the multiplier, with ten points serving as a common reference allocation.

\begin{table}[H]
\centering
\caption{Illustrative payoffs across decision methods and crisis conditions}
\label{tab:payoff-matrix}
\begin{threeparttable}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.23\textwidth}XX}
\toprule
Decision method & Crisis: leader productivity premium & Recovery: equal productivity \\
\midrule
Citizens decide (\Dproc) &
\(m=1.50\). If every citizen places 10 points in the fund, each earns
\[
\pi_i^D=20-10+\frac{1.50}{5}(5\times10)=25.
\] &
\(m=2.50\). With the same choices, each earns
\[
\pi_i^D=20-10+\frac{2.50}{5}(5\times10)=35.
\] \\
\addlinespace
A leader decides (\Aproc) &
\(m=2.50\). If the leader requires 10 points and sets \(\rho=0\), each earns
\[
\pi_i^A=20-10+\frac{2.50}{5}(5\times10)=35.
\] &
\(m=2.50\), unchanged. The same leader decision again gives each citizen
\[
\pi_i^A=35.
\] \\
\bottomrule
...
\item \emph{Note:} The persistence condition retains the crisis column in Block 2. If the leader moves points to a personal account (\(\rho>0\)), nonleaders receive less and the leader receives that transfer in addition to the common return, as defined in \cref{eq:executive-nonleader,eq:executive-leader}.
\end{tablenotes}
\end{threeparttable}
\end{table}

The matrix maps the incentive structure onto the hypotheses. In the example, the leader-led method produces a ten-point payoff advantage during the crisis, and recovery equalizes the productivity rate at \(m=2.50\). At that common rate, a point voluntarily placed in the citizen-led fund returns \(2.50/5=0.50\) points to the citizen who placed it. The private incentive to free-ride therefore remains, while a leader can coordinate provision by requiring equal allocations. Leader control combines this coordination advantage with authority over individual choices and access to a personal transfer. For example, if the leader requires ten points and moves ten fund points to a personal account, each nonleader earns 30, the leader earns 40, and a citizen's ex ante expected payoff before leader selection is 32 under symmetric selection probabilities.

Accordingly, H1 concerns support for leader control when it combines a productivity premium with compulsory coordination. H4 concerns whether citizens return to collective control once that experimentally assigned premium expires and the leader's coordination advantage remains. Because both methods use \(2.50\) under recovery, a recovery-induced increase in \Dproc{} ballots records a consequential preference for collective control under continuing free-riding risk. The persistence pools identify this change in revealed institutional preference against an environment in which leader control retains both advantages.
...
Paid rounds & 10 per block (20 total), preceded by unpaid practice. {\color{red}(pre-reg 1st round; ``hot decision.'')} \\
Group and matching pool & 5-person groups; 10-person treatment-matched pools {\color{red}maybe it builds reputation? think; find reference} \\
...
Performance payment & One randomly selected game round from each block {\color{red}(find reference)} \\
...
\newpage
\begin{plainlanguagebox}
The primary causal estimand ($\tau_R$) asks: among participants who chose ``A leader decides'' in the final round of Block 1 ($V_{i,1,10}=\Aproc$, summarized by $A_{i1}=1$), how much does assignment to recovery ($T_g=1$), rather than persistence ($T_g=0$), increase the probability that they return to ``Citizens decide'' in the final round of Block 2 ($V_{i,2,10}=\Dproc$), thereby constituting a democratic reversal ($R_i=1$)? Under recovery, this return occurs after both methods reach the same productivity rate and while the leader-led method retains its ability to require equal allocations. The outcome therefore captures a consequential choice for collective control under continuing free-riding risk. Ideally, we would compare each participant's decision under recovery ($R_i(1)$) with the decision that the same participant would have made under persistence ($R_i(0)$). Because both outcomes cannot be observed for the same person, random assignment allows us to estimate this effect by comparing reversal rates between conditions. Participants are analyzed according to their pool's assigned condition ($T_g$), and statistical uncertainty is calculated at the pool level.
\end{plainlanguagebox}
...
Process outcomes occur at baseline, after Block 1, and after Block 2. They include perceived public-service effectiveness, perceived risk to constraints, the seven-item constraint index, round-level ballots, fund decisions, leader decisions, and realized payoffs. Battery items are randomized once per participant and then stored, preventing re-randomization on refresh. Any decomposition through perceived effectiveness or exposure to leader behavior will be labeled exploratory unless supported by a preregistered causal mediation strategy. The clean confirmatory claim remains the pool-randomized effect of recovery on reversal.
